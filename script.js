document.addEventListener('DOMContentLoaded', () => {
    ...
    // ✅ ADD: Get ipcRenderer at the top for use throughout
    ...
    // ✅ UPDATED: Save Handler with better feedback for temp save
    async function saveAndCopyIcon() {
        ...
        // Use IPC to save file to temp folder
        ...
        if (result.success) {
            ...
            // ✅ UPDATED: Better feedback message for temp save
            alert(`✓ Icon saved to temp folder!\n\nFile path copied to clipboard.\nReady to upload to Notion!\n\n📁 ${result.path}`);

            console.log('Icon saved to temp:', result.path);
        } else {
    ...
    // ✅ ADD: Settings modal functionality (if you added settings button)
    ...
    if (settingsButton) {
        settingsButton.addEventListener('click', async () => {
            ...
            const currentShortcut = await ipcRenderer.invoke('get-shortcut');
            shortcutInput.value = currentShortcut.replace('CommandOrControl', 'Ctrl');
    ...
    if (saveSettings) {
        saveSettings.addEventListener('click', async () => {
            ...
            if (!shortcut) {
                showSettingsStatus('Please enter a shortcut', false);
                return;
            }

            // Convert to Electron format
            shortcut = shortcut.replace('Ctrl', 'CommandOrControl');

            const result = await ipcRenderer.invoke('update-shortcut', shortcut);

            if (result.success) {
                showSettingsStatus(`Shortcut updated to ${shortcut.replace('CommandOrControl', 'Ctrl')}!`, true);
                setTimeout(() => {
                    settingsModal.style.display = 'none';
                }, 1500);
            } else {
                showSettingsStatus(result.error, false);
            }
        });
    }
    ...
    function showSettingsStatus(message, isSuccess) {
        ...
        settingsStatus.className = isSuccess ? 'success' : 'error';
